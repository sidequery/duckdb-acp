...
struct AcpOperatorExtension : public OperatorExtension {
    AcpOperatorExtension() : OperatorExtension() { Bind = acp_bind; }

    std::string GetName() override { return "acp"; }
...
ParserExtensionPlanResult acp_plan(ParserExtensionInfo *, ClientContext &,
...
struct AcpParseData : ParserExtensionParseData {
    ...
    unique_ptr<ParserExtensionParseData> Copy() const override {
        return make_uniq_base<ParserExtensionParseData, AcpParseData>(
            statement->Copy());
    }

    virtual string ToString() const override { return "AcpParseData"; }

    AcpParseData(unique_ptr<SQLStatement> statement)
        : statement(std::move(statement)) {}
};

class AcpState : public ClientContextState {
public:
    explicit AcpState(unique_ptr<ParserExtensionParseData> parse_data)
